...
public class ProcessMixedService : LotteryBaseService<ProcessMixedService>, IProcessMixedService
{
    ...
    public async Task<List<Data.Entities.Ticket>> ProcessV2(ProcessMixedTicketV2Model model, ProcessValidationTicketV2Model processValidation, ProcessValidationTicketDetailV2Model itemModel)
    {
        var noOfNumbers = model.BetKindId.GetNoOfNumbers();
        var enableStats = _ticketProcessor.EnableStats(model.BetKindId);
        var dictSubBetKindIds = _ticketProcessor.GetSubBetKindIds(model.BetKindId);
        ...
        //  Get Company Odds, Agent Odds
        var agentOddsValue = await _processTicketService.GetAgentMixedOdds(itemModel.BetKind.Id, dictSubBetKindIds.Keys.ToList(), processValidation.Player.SupermasterId, processValidation.Player.MasterId, processValidation.Player.AgentId) ?? throw new BadRequestException(ErrorCodeHelper.ProcessTicket.CannotReadAgentOdds);
        ...
        foreach (var item in dictSubBetKindIds)
        {
            ...
            else
            {
                ...
                var ticket = CreateParentTicketV2(correlationId, processValidation, itemModel, betKindId, currentBetKind, playerOddsValue, normalizedNumbers, positionTakings);
                var childTickets = new List<Data.Entities.Ticket>();
                foreach (var itemSubsets in subsets)
                {
                    var subsetNumbers = itemSubsets.OrderBy(f => f).ToList();
    ...
    private Data.Entities.Ticket CreateParentTicket(Guid correlationId, ProcessValidationTicketModel processValidation, int betKindId, BetKindModel betKind, decimal playerOddsValue, string normalizedNumbers, List<AgentPositionTakingModel> positionTakings)
    {
        ...
        return new Data.Entities.Ticket
        {
            ...
            ChannelId = processValidation.Channel != null ? processValidation.Channel.Id : -1,
            ChoosenNumbers = normalizedNumbers,
            ShowMore = true,
    ...
    private Data.Entities.Ticket CreateSingleTicketV2(Guid correlationId, ProcessValidationTicketV2Model processValidation, ProcessValidationTicketDetailV2Model itemModel, BetKindModel betKind, decimal points, decimal playerOddsValue, string normalizedNumbers, List<AgentPositionTakingModel> positionTakings)
    {
        ...
        return new Data.Entities.Ticket
        {
            ...
            MatchId = processValidation.Match.MatchId,
            KickOffTime = processValidation.Match.KickoffTime,
            RegionId = itemModel.BetKind.RegionId,
            ChannelId = itemModel.Channel.Id,
            ChoosenNumbers = normalizedNumbers,
            CorrelationCode = correlationId,
            RewardRate = betKind.Award,
            Stake = points,
            IsLive = itemModel.Metadata.IsLive,
            Prize = itemModel.Metadata.Prize,
    ...
    private Data.Entities.Ticket CreateParentTicketV2(Guid correlationId, ProcessValidationTicketV2Model processValidation, ProcessValidationTicketDetailV2Model itemModel, int betKindId, BetKindModel betKind, decimal playerOddsValue, string normalizedNumbers, List<AgentPositionTakingModel> positionTakings)
    {
        var agentPt = 0m;
        var agentPostionTaking = positionTakings.Find(f => f.AgentId == processValidation.Player.AgentId);
        if (agentPostionTaking != null) agentPt = agentPostionTaking.PositionTaking;

...
public class ProcessLiveService : LotteryBaseService<ProcessLiveService>, IProcessLiveService
{
    ...
    private readonly IBetKindService _betKindService;
    ...
    public ProcessLiveService(ILogger<ProcessLiveService> logger, IServiceProvider serviceProvider, IConfiguration configuration, IClockService clockService, ILotteryClientContext clientContext, ILotteryUow lotteryUow,
        ...
        IBetKindService betKindService) : base(logger, serviceProvider, configuration, clockService, clientContext, lotteryUow)
    {
    ...
    public async Task<(Data.Entities.Ticket, List<Data.Entities.Ticket>)> Process(ProcessTicketModel model, ProcessValidationTicketModel processValidation)
    {
        ...
        var replacedByBetKindId = _betKindService.GetReplacedBetKind(originBetKindId.ToEnum<Enums.BetKind>()).ToInt();
        ...
        //  Get Outs
        var outs = await _processTicketService.GetOuts(processValidation.Player.PlayerId, processValidation.Match.MatchId, originBetKindId, model.Numbers.Select(f => f.Number).ToList());
        ...
        if (model.Numbers.Count == 1)
        {
            ...
            //  Agent
            ticket.AgentOdds = ticket.PlayerOdds;
            ticket.AgentPayout = _ticketProcessor.GetPayoutByNumber(processValidation.BetKind, thisNumber.Point, ticket.AgentOdds.Value);

            //  Master
            ticket.MasterOdds = ticket.PlayerOdds;
            ticket.MasterPayout = _ticketProcessor.GetPayoutByNumber(processValidation.BetKind, thisNumber.Point, ticket.MasterOdds.Value);

            //  Supermaster
            ticket.SupermasterOdds = ticket.PlayerOdds;
            ticket.SupermasterPayout = _ticketProcessor.GetPayoutByNumber(processValidation.BetKind, thisNumber.Point, ticket.SupermasterOdds.Value);
        ...
        {
            { originBetKindId, pointByNumbers }
        });
    ...
    public async Task<(Data.Entities.Ticket, List<Data.Entities.Ticket>)> ProcessV2(ProcessTicketDetailV2Model model, ProcessValidationTicketV2Model processValidation)
    {
        var originBetKindId = model.BetKindId;
        var replacedByBetKindId = _betKindService.GetReplacedBetKind(originBetKindId.ToEnum<Enums.BetKind>()).ToInt();
        var enableStats = _ticketProcessor.EnableStats(model.BetKindId);
        ...
        if (setting == null) throw new BadRequestException(ErrorCodeHelper.ProcessTicket.CannotReadBetSetting);
        if (refreshSettingCache) await _playerSettingService.BuildSettingByBetKindCache(processValidation.Player.PlayerId, model.BetKindId, setting);

        //  Player Odds by RunningMatch, BetKind and Numbers
        var playerOdds = await _oddsService.GetInitialOdds(processValidation.Player.PlayerId, originBetKindId);

        //  Get agent position taking
        var agentPts = await _agentPositionTakingService.GetAgentPositionTakingByAgentIds(new List<int> { model.BetKindId }, new List<long> { processValidation.Player.SupermasterId, processValidation.Player.MasterId, processValidation.Player.AgentId });
        if (!agentPts.TryGetValue(model.BetKindId, out List<AgentPositionTakingModel> positionTaking)) positionTaking = new List<AgentPositionTakingModel>();
        ...
        var ticket = new Data.Entities.Ticket
        {
            ...
            State = TicketState.Waiting.ToInt(),
            IsLive = metadataDetail.Metadata.IsLive,
            Prize = metadataDetail.Metadata.Prize,
            Position = metadataDetail.Metadata.Position,
        ...
        if (model.Numbers.Count == 1)
        {
            ...
            ticket.PlayerOdds = oddsByNumberDetail.Buy;
            ticket.PlayerPayout = _ticketProcessor.GetPayoutByNumber(metadataDetail.BetKind, thisNumber.Point, ticket.PlayerOdds.Value);

            //  Agent
            ticket.AgentOdds = ticket.PlayerOdds;
            ticket.AgentPayout = _ticketProcessor.GetPayoutByNumber(metadataDetail.BetKind, thisNumber.Point, ticket.AgentOdds.Value);

            //  Master
            ticket.MasterOdds = ticket.PlayerOdds;
            ticket.MasterPayout = _ticketProcessor.GetPayoutByNumber(metadataDetail.BetKind, thisNumber.Point, ticket.MasterOdds.Value);

            //  Supermaster
            ticket.SupermasterOdds = ticket.PlayerOdds;
            ticket.SupermasterPayout = _ticketProcessor.GetPayoutByNumber(metadataDetail.BetKind, thisNumber.Point, ticket.SupermasterOdds.Value);
        ...
        else
        {
            ...
            foreach (var item in numbers)
            {
                ...
                var playerPayout = _ticketProcessor.GetPayoutByNumber(metadataDetail.BetKind, item.Point, oddsByNumberDetail.Buy);
                var companyPayout = playerPayout;
        ...
        if (enableStats) await _processTicketService.BuildStatsByMatchBetKindAndNumbers(processValidation.Match.MatchId, model.BetKindId, pointByNumbers, payoutByNumbers, realPayoutByNumbers);

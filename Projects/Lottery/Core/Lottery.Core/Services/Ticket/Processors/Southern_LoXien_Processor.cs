...
public class Southern_LoXien_Processor : AbstractBetKindProcessor
{
    private readonly Dictionary<int, int> _subBetKindIds = new()
    {
        { Enums.BetKind.Southern_Xien2.ToInt(), 2 },
        { Enums.BetKind.Southern_Xien3.ToInt(), 3 },
        { Enums.BetKind.Southern_Xien4.ToInt(), 4 }
    };

    public Southern_LoXien_Processor(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }

    public override int BetKindId { get; set; } = Enums.BetKind.Southern_LoXien.ToInt();

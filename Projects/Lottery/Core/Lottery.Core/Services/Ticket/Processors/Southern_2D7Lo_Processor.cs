...
public class Southern_2D7Lo_Processor : AbstractBetKindProcessor
{
    private const int _startedPrize = 4;
    private const int _specialPrize = 9;

    public Southern_2D7Lo_Processor(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }

    public override int BetKindId { get; set; } = Enums.BetKind.Southern_2D7Lo.ToInt();

...
public class Southern_4D16Lo_Processor : AbstractBetKindProcessor
{
    private const int _resultFromPrize = 3;
    private const int _resultToPrize = 9;

    public Southern_4D16Lo_Processor(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }

    public override int BetKindId { get; set; } = Enums.BetKind.Southern_4D16Lo.ToInt();
    ...
    {
        var rs = result.Where(f => f.Prize >= _resultFromPrize && f.Prize <= _resultToPrize).SelectMany(f => f.Results).Select(f => f.Result).ToList();

...
public class Southern_Mixed_Xien3_Processor : AbstractBetKindProcessor
{
    private const int _numberOfChannels = 2;

    public Southern_Mixed_Xien3_Processor(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }

    public override int BetKindId { get; set; } = Enums.BetKind.Southern_Mixed_Xien3.ToInt();
    ...
    private bool CheckOnPairChannels(string choosenNumbers, Dictionary<int, List<PrizeMatchResultModel>> results, List<int> calculatedChannelIds, out int times)
    {
        ...
        var listNumbers = choosenNumbers.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        if (listNumbers.Length != 3) return false;
        ...
        foreach (var channelId in calculatedChannelIds)
        {
            ...
            if (timesOfFirstNumber != null && timesOfSecondNumber != null && timesOfThirdNumber != null)
            {
                dictResults[channelId] = Math.Max(Math.Max(timesOfFirstNumber.Count, timesOfSecondNumber.Count), timesOfThirdNumber.Count);
                continue;
            }
            break;
        }

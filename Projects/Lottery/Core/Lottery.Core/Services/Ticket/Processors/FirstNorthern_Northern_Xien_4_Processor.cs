...
public class FirstNorthern_Northern_Xien_4_Processor : AbstractBetKindProcessor
{
    private const int _startedPrize = 2;    //  Exclude Than Tai
    private const int _endPrize = 9;

    public FirstNorthern_Northern_Xien_4_Processor(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }

    public override int BetKindId { get; set; } = Enums.BetKind.FirstNorthern_Northern_Xien4.ToInt();
    ...
    public override CompletedTicketResultModel Completed(CompletedTicketModel ticket, List<PrizeMatchResultModel> result)
    {
        ...
        if (ticket.Children.Count == 0)
        {
            ...
            if (isWon)
            {
                ...
                //}
                //if (timesOfFourthNumber.Count > 1)
                //{
                //    times += timesOfFourthNumber.Count;
                //    mixedTimes[timesOfFourthNumber.Key] = timesOfFourthNumber.Count;
                //}
